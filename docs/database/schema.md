...
`firestore.indexes.json` — 10 composite indexes deployed:
...
| `friend_requests` | `toUid ASC, createdAt DESC` | Friends: legacy incoming requests query (all statuses) |
| `friend_requests` | `toUid ASC, status ASC, createdAt DESC` | Friends: pending incoming requests — `watchIncomingRequests` (`status == pending` filter) |

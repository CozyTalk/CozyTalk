...
| `incomingRequests` | `List<FriendRequest>` | Pending friend requests addressed to the current user; filtered to `status == pending` at the Firestore query level, sorted newest-first, capped at 10 |
...
**Outgoing requests stream** — `FriendsNotifier` subscribes `_outgoingRequestsSub` to `watchOutgoingRequests()` on `build()`. The stream feeds `outgoingRequests` in state. Three helpers on `FriendsState` derive add-friend button state without extra queries: `hasSentRequestTo(uid)` — `true` when a pending outgoing request exists for that UID; `isFriend(uid)` — `true` when a `Friend` entry exists for that UID; `hasIncomingPendingFrom(uid)` — `true` when a pending incoming request from that UID is present. `cancelFriendRequest(toUid)` delegates to the `CancelFriendRequest` usecase.

**Mutual pending auto-accept** — when `_requestsSub` or `_outgoingRequestsSub` emits and a mutual pending state is detected (incoming request from X and an outgoing request to X both exist), `FriendsNotifier` calls `_writeAccept` automatically without user interaction. Checks are performed in both handlers to handle whichever stream fires first. A `_autoAcceptingIds` set prevents duplicate writes before the stream settles. `sendFriendRequest` also performs the same check so that tapping "Add Friend" from the chat screen on a user who already sent a request auto-accepts it immediately.

**`removeFriend` request reset** — `FriendsDatasourceImpl.removeFriend` deletes the `friendships` doc and then attempts to reset the corresponding `friend_requests` doc back to `status: 'pending'` by trying both possible ID directions (`uid1_uid2` and `uid2_uid1`). This ensures the receiver's notification card does not get stuck in an accepted state after unfriending.
...
`NotificationScreen` (`screens/notification_screen.dart`) — shows only pending friend requests (datasource filters to `status == pending`; screen additionally hides cards from users already in the `friends` list to handle race conditions). Tapping **Accept** calls `acceptRequest(request)` immediately; tapping **Decline** calls `declineRequest(request)` immediately — both write to Firestore directly with no local queue. The card disappears when the Firestore stream emits the updated status. Up to 10 pending requests are shown, newest-first. No deferred-write or undo mechanism.

**"Add Friend" / Unfriend in active chat sessions** — `ChatScreen` and `GroupChatScreen` derive the add-friend button state from `friendsNotifierProvider` using `AddFriendStatus` enum (`notAdded` / `pending` / `friends`). Three distinct visual states: green (`notAdded` → send request), amber (`pending` → cancel), pink (`friends` → unfriend). `_partnerFriendStatus()` / `_friendStatus(uid)` check `isFriend()` and `hasSentRequestTo()`. When `pending`, tapping calls `cancelFriendRequest(toUid)`; when `friends`, tapping calls `_unfriend(uid)` which shows a confirm dialog before calling `removeFriend(friendshipId)`. `GroupChatScreen` also wires unfriend through `MembersPanelBody.onUnfriend` and the member avatar / message bubble `UserProfileDialog` call sites. Errors surface as SnackBar via `ref.listen`.

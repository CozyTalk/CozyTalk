...
- `watchFriendLastMessage(chatRoomId)`: streams `({String text, DateTime? timestamp})` from the latest message in `friend_messages/{chatRoomId}/messages` (descending by timestamp, limit 1). Emits `(text: '', timestamp: null)` when no messages exist.
...
| `unreadChatRoomIds` | `Set<String>` | chatRoomIds that received a new message while the screen was not open; cleared by `markChatAsRead(chatRoomId)` |

...
- `RemoveFromQueue` returns the updated `JukeboxRoomState?` (null when the last track is removed and the queue is cleared); `removeFromQueue()` applies it immediately so `addUrl()`'s queue-full guard reflects the true count before the RTDB round-trip completes. An OOB index guard in the notifier (`index < 0 || index >= roomState.queue.length`) ensures the use case is never called with an invalid index, preventing an OOB null return from silently wiping `roomState`.
- `SongPanelBody` (in `song_dialog.dart`) defers WebView creation until the panel has a real layout surface. A `ref.listen` on `currentTrack` arms the WebView via a post-frame callback as soon as RTDB reports an active track — even before the Song panel is opened — so playback starts automatically on room entry.

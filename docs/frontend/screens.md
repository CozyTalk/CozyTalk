...
| `ChatScreen` | `screens/chat_screen.dart` | `/chat` | ConsumerStatefulWidget | ✅ integrated — wired to `chatNotifierProvider`; GIF via Giphy API; topic card shows shuffler's display name above the card image for cards received from the partner; add-friend button uses `AddFriendStatus` enum derived from `friendsNotifierProvider` — three visual states: green (`notAdded` → send request), amber (`pending` → cancel), pink (`friends` → unfriend with confirm dialog); mutual pending auto-accepts when tapping Add Friend on a user who already sent a request; **Skip Room** button in side column — shows confirmation dialog, calls `endSession()`, then `pushReplacementNamed` to `/finding-room` with original `roomName`, `bgImage`, and `roomType: '1v1'`; `_isSkipping` flag suppresses the disconnect-listener auto-pop during skip navigation |
| `GroupChatScreen` | `screens/group_chat_screen.dart` | `/group-chat` | ConsumerStatefulWidget | ✅ integrated — wired to `chatNotifierProvider` + `matchmakingNotifierProvider` (isLocked); GIF via Giphy API; topic card shows shuffler's display name above the card image for cards received from other participants; add-friend button per member uses `AddFriendStatus` enum — three visual states matching `ChatScreen` (green/amber/pink); `MembersPanelBody` wires `onUnfriend` alongside `onAddFriend`/`onCancelRequest`; unfriend also wired to member-avatar dialog and message-bubble dialogs; **Skip Room** button in side column — shows confirmation dialog, calls `leaveRoom()` + `forceDisconnect()`, then `pushReplacementNamed` to `/finding-room` with resolved `roomName`/`bgImage` from live `currentRoom.backgroundTheme` (falls back to route args); `joinById` roomType remapped to `'group'`; `_isSkipping` flag suppresses the disconnect-listener auto-pop during skip navigation |
...
| `NotificationScreen` | `screens/notification_screen.dart` | `/notification` | ConsumerStatefulWidget | ✅ integrated — shows only pending friend requests (`watchIncomingRequests` filters to `status == pending` at query level; screen further hides cards from existing friends to handle race conditions); Accept/Decline write to Firestore immediately via `acceptRequest`/`declineRequest` — no local queue, no deferred commit, no undo; card disappears when stream fires after status change; up to 10 pending requests shown newest-first |

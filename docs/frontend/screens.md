...
| `NotificationScreen` | `screens/notification_screen.dart` | `/notification` | ConsumerStatefulWidget | ✅ integrated — `friendsNotifierProvider.incomingRequests` (all statuses, newest-first, capped at 10); deferred-write: Accept/Decline queued locally in `pendingActions`, committed to Firestore on Back; grey-card undo reverts queued actions locally or committed actions via Firestore (requires `friend_requests` security rule deployment); App Update static card |

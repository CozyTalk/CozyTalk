...
export const sendMessage = onCall(
  ...
  async (request) => {
    ...
    // to active_sessions (proto-session backward compat).
    const roomSnap = await db.collection("rooms").doc(sessionId).get();
    const sessionRef = roomSnap.exists
      ? db.collection("rooms").doc(sessionId)
      : db.collection("active_sessions").doc(sessionId);

    // Ensure session key exists atomically — rooms have it at creation;
    // legacy active_sessions generate it on the first message.
    let encryptionKey = "";
    ...
    // Fall back to RTDB presence name (set by joinProtoSession for all session types,
    // generates adj+animal names for anonymous users like "Brave Bear").
    if (displayName === "Anonymous") {
      const rtdb = admin.database();
      const presenceSnap = await rtdb.ref(`presence/${sessionId}/${uid}`).get();
      if (presenceSnap.exists()) {
        displayName = (presenceSnap.val() as string) || "Anonymous";
      }
    }
